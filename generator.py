...
class Hunyuan3D2mvGenerator(BaseGenerator):
    ...
    def _load_variant(self, variant):
        ...
        # Add fast and standard models to the variant list
        if variant == "hunyuan3d-dit-v2-mv-fast":
            subfolder = _SUBFOLDERS["hunyuan3d-dit-v2-mv-fast"]
        elif variant == "hunyuan3d-dit-v2-mv":
            subfolder = _SUBFOLDERS["hunyuan3d-dit-v2-mv"]
...
class Hunyuan3D2mvTurboGenerator(Hunyuan3D2mvGenerator):
    ...
    def _load_variant(self, variant):
        if variant == self.MODEL_VARIANT:
            subfolder = _SUBFOLDERS[self.MODEL_VARIANT]
            self._pipeline = self._Pipeline.from_pretrained(
                str(self.model_dir),
                subfolder=subfolder,
                use_safetensors=True,
